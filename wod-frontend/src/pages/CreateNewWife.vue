        ...
        <v-stepper v-model="currentStep" :items="steps" hide-actions>
            <template v-slot:item.1>
                <v-row dense>
                    ...
                    >
                        <v-card-title>{{ wife.name }}</v-card-title>
                        <v-card-text>{{ wife.text }}</v-card-text>
                    </v-card>
                    </v-col>
                </v-row>
                <v-card-actions>
...
<style scoped>
    .selected-card {
    border: 2px solid #42a5f5; /* 選中卡片的邊框顏色 */
    background-color: rgba(66, 165, 245, 0.1); /* 選中卡片的背景色 */
    }
</style>